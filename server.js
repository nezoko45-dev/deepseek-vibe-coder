...
async function askGroq(prompt) {
  if (!GROQ_API_KEY) throw new Error("Missing Groq API key. Add groqApiKey to config.json.");
  ...
  if (!response.ok) {
    throw new Error(`Groq ${response.status}: ${text.slice(0, 1600)}`);
  }
...
async function vibe(body) {
  ...
  const pr = await createPullRequest(
    ...
    branch,
    base,
    plan.prTitle || `Groq coding: ${task.slice(0, 60)}`,
    plan.prBody || `Groq generated this change from the task:\n\n${task}`
  );

  return {
    ok: true,
    summary: plan.summary || "Changes generated by Groq.",
    repo,
    base,
    branch,
    commit: commit.sha,
    pullRequestUrl: pr.html_url,
    changes: changes.map(x => ({ path: x.path, action: x.action }))
  };
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && url.pathname === "/vibe") {
      return send(res, 200, await vibe(await readBody(req)));
    }
