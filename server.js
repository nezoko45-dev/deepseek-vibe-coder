...
const PLAN_SCHEMA = {
  ...
  properties: {
    summary: { type: "string" }, commitMessage: { type: "string" }, prTitle: { type: "string" }, prBody: { type: "string" },
    files: { type: "array", items: { type: "object", additionalProperties: false, properties: {
      path: { type: "string" }, action: { type: "string", enum: ["create", "update", "delete"] }, content: { type: ["string", "null"] }
    }, required: ["path", "action", "content"] } }
  },
  required: ["summary", "commitMessage", "prTitle", "prBody", "files"]
};
...
async function askGroq(prompt) {
  ...
  const response = await fetch(GROQ_URL, {
    ...
    body: JSON.stringify({
      ...
      response_format: { type: "json_schema", json_schema: { name: "github_coding_plan", strict: true, schema: PLAN_SCHEMA } }
    })
  });
  const text = await response.text();
  if (!response.ok) throw new Error(`Groq ${response.status}: ${text.slice(0, 1600)}`);
  let data; try { data = JSON.parse(text); } catch { throw new Error("Groq returned invalid API JSON."); }
  const choice = data?.choices?.[0];
  const content = choice?.message?.content;
  if (!content) throw new Error(`Groq returned no coding plan content (finish_reason=${choice?.finish_reason || "unknown"}).`);
  return parseAgentJson(content);
}

async function readBody(req) { let text = ""; for await (const chunk of req) { text += chunk; if (text.length > 200000) throw new Error("Request body is too large."); } return JSON.parse(text || "{}"); }

async function vibe(body) {
  const task = String(body.task || "").trim(); const repo = String(body.repo || DEFAULT_REPO).trim(); const base = String(body.base || "main").trim();
  ...
  const prompt = [`USER TASK:\n${task}`, `TARGET REPOSITORY: ${repo}`, `BASE BRANCH: ${base}`, `REPOSITORY FILES:\n${JSON.stringify(snapshot.files)}`, "Return the complete coding plan as the required JSON object. The files array must contain every file that needs to be created, updated, or deleted. For create/update, content must contain the complete file text. If no change is needed, return an empty files array."].join("\n\n");
  const plan = await askGroq(prompt);
  if (!plan || !Array.isArray(plan.files) || plan.files.length === 0) throw new Error("Groq produced no file changes. Try making the task more specific about what should be changed.");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/status") {
      let groqOnline = false; if (GROQ_API_KEY) { try { const r = await fetch("https://api.groq.com/openai/v1/models", { headers: { authorization: `Bearer ${GROQ_API_KEY}` } }); groqOnline = r.ok; } catch {} }
      return send(res, 200, { name: "Groq GitHub Vibe Coder", status: "online", groqOnline, model: GROQ_MODEL, githubConfigured: Boolean(GITHUB_TOKEN), apiKeyRequired: true, deepgram: false, cloudflare: false, ollama: false });
    }
    if (req.method === "POST" && url.pathname === "/vibe") return send(res, 200, await vibe(await readBody(req)));
    return send(res, 404, { error: "Not found" });
  } catch (error) { console.error(error); return send(res, 500, { error: error instanceof Error ? error.message : String(error) }); }
});
